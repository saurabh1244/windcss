...
def post_user(request):
    if request.method == 'POST':
        ...
        blog_name = request.POST['blog_name']
        blog_code = request.POST['blog_code']
        user_html_data = request.POST['blog_desc']
        blog_desc = f'<xmp>\n{user_html_data}\n</xmp>'

        obj = blogx.objects.create( blog_name=blog_name, blog_code=blog_code, blog_desc=blog_desc)
        obj.save()
        return redirect('/')
